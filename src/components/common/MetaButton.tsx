...
export const MetaButton: React.FC = () => {
  ...
  useEffect(() => {
    if (!APP_ID) console.error('❌ Falta NEXT_PUBLIC_META_APP_ID')
    if (!CONFIG_ID) console.error('❌ Falta NEXT_PUBLIC_META_CONFIG_ID')
  }, [])
  ...
  const fbLoginHandler = (response: any) => {
    void (async () => {
      if (response?.authResponse) {
        ...
        console.warn('No authResponse en la respuesta de FB:', response)
      }
    })().catch((err) => console.error('fbLoginCallbackAsync error:', err))
  }

  const launchWhatsAppSignup = () => {
    if (!fbReady) {
      alert('Facebook SDK no está listo todavía.')
      return
    }
    if (!(window as any).FB) {
      return
    }
    ;(window as any).FB.login(fbLoginHandler, {
      config_id: CONFIG_ID,
      response_type: 'code',
        ...
        <div className="grid p-1 text-center">
          <div className="p-4 mt-3">
            <h2 className="text-md font-bold text-gray-600 text-lg dark:text-white/90">
              Conectar cuenta Meta Bussiness
            </h2>
          </div>
        </div>
